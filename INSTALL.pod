...
Set up lighttpd with FastCGI enabled (master only)

Install perl modules FCGI and JSON (master only)

Install libwww-perl (master and all workers)

Install Digest::SHA1 (master and all workers)
